...
# class for analyzing sync pulse alignmentes in System 1 data (on cmlreaders)
class sync_pulse_aligner_cml:
    ...
    def __init__(self, subject, subject_alias, experiment, original_experiment, session, original_session, sync_txt):    # initial attributes from valid_sync_files.csv
    ...
    def _deal_with_changes(self, exp_change, sess_change):
        if exp_change:
        ...
        if sess_change:
            sess_match = self.original_session
        else:
    ...
    def _load_beh_syncs(self):
        ...
        re_implant = False
        # infer from the subject_alias --> make sure this is note NaN
        if type(self.subject_alias) == str and self.subject != self.subject_alias and '_' in self.subject_alias:
            re_implant = True
        ...
        sess_change = False
        if not math.isnan(self.original_session) and self.original_session != self.session:
            sess_change = True
            
        exp_match, sess_match = self._deal_with_changes(exp_change, sess_change)   # deal with experiment and session changes
        if self.original_experiment == 'CatFR1':        # handle edge case
            exp_match = 'catFR1'
            
        # not a re-implant, use subject
        if not re_implant:
            behdir = f'/data/eeg/{self.subject}/behavioral/{exp_match}/session_{sess_match}/'
        else:      # re-implant, use subject alias
            behdir = f'/data/eeg/{self.subject_alias}/behavioral/{exp_match}/session_{sess_match}/'
            
        log_files = glob(behdir+'session.json*') + glob(behdir+'eeg.eeglog') + glob(behdir+'eeg.eeglog.up')
        if len(log_files) > 0:
            log_file = log_files[0]
        else:
            raise RuntimeError('Unable to locate behavioral sync pulse log.')
        ...
        if log_file.count('json') > 0:
            ...
            syncs = [float(line[0]) for line in split_lines if line[2] in ('CHANNEL_0_UP', 'ON', 'UP')]     # YC logs us 'UP'
    ...
    def _remove_gaps(self):
        ...
        """
        if len(gap_idx) >1:    # only case is 2 gaps where the 2nd is always at index 99
            # remove the end of the arrays (after 2nd gap)
            bsd = bsd[:gap_idx[1]]
            esd = esd[:gap_idx[1]]
            no_gap_idx = no_gap_idx[no_gap_idx < gap_idx[1]]
            self.matching_beh_syncs = self.matching_beh_syncs[:gap_idx[1]]
            self.matching_eeg_syncs = self.matching_eeg_syncs[:gap_idx[1]]
            print("Multiple gaps detected, removing values from after 2nd gap.")
            #raise ValueError("Multiple gaps detected.")
         """
        
        # no gaps, not modification necessary
        if len(gap_idx) == 0:
        ...
        else:       # only case is 2 gaps where the 2nd is always at index 99
            # remove indices where there is a big gap in the syncs
            beh_syncs_diff_clean = bsd[no_gap_idx]
            eeg_syncs_diff_clean = esd[no_gap_idx]
            
            # modify raw matching times
            # for all times after gap, subtract first time after gap, add final time before gap, add mean difference times before gap
            section_1_beh_syncs = self.matching_beh_syncs[:gap_idx[0] + 1]
            section_1_eeg_syncs = self.matching_eeg_syncs[:gap_idx[0] + 1]
            section_2_beh_syncs = self.matching_beh_syncs[gap_idx[0]+1: gap_idx[1] + 1]
            section_2_eeg_syncs = self.matching_eeg_syncs[gap_idx[0]+1: gap_idx[1] + 1]
            section_3_beh_syncs = self.matching_beh_syncs[gap_idx[1] + 1:]
            section_3_eeg_syncs = self.matching_eeg_syncs[gap_idx[1] + 1:]
            
            # modfiy middle section first
            section_2_beh_syncs = section_2_beh_syncs - section_2_beh_syncs[0] + section_1_beh_syncs[-1] + np.round(np.mean(np.diff(section_1_beh_syncs)), 0)
            section_2_eeg_syncs = section_2_eeg_syncs - section_2_eeg_syncs[0] + section_1_eeg_syncs[-1] + np.round(np.mean(np.diff(section_1_eeg_syncs)), 0)
            # final section
            section_3_beh_syncs = section_3_beh_syncs - section_3_beh_syncs[0] + section_2_beh_syncs[-1] + np.round(np.mean(np.diff(section_2_beh_syncs)), 0)
            section_3_eeg_syncs = section_3_eeg_syncs - section_3_eeg_syncs[0] + section_2_eeg_syncs[-1] + np.round(np.mean(np.diff(section_2_eeg_syncs)), 0)
            
            # concatenate
            matching_beh_syncs_clean = np.concatenate((section_1_beh_syncs, section_2_beh_syncs, section_3_beh_syncs))
            matching_eeg_syncs_clean = np.concatenate((section_1_eeg_syncs, section_2_eeg_syncs, section_3_eeg_syncs))
